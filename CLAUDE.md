...
- **Plan-first workflow**: Before executing non-trivial tasks, write a plan in `TODO.md` (checklist format), spawn relevant review agents in parallel to reach consensus, then proceed. Mark items done when complete.
...
- **Parallel execution**: Always find concurrent execution opportunities. For multi-service or multi-file tasks, spawn 3-5 agents in parallel (one per service group). Use `run_in_background: true` for agents whose results are not needed immediately. Only go sequential when a later step genuinely depends on an earlier step's output.
...
- Redis type: `Redis` unparameterized (not `Redis[str]`). `hmget()` requires list form: `hmget(key, [fields])`.
- Config source of truth: `.env.example` for env vars, `pyproject.toml` for tool config.
