...
**OpggSource** (`lol-pipeline-common/src/lol_pipeline/sources/opgg/source.py`) performs a real on-demand fetch when `data_type=MATCH` and a PUUID is present. `fetch()` calls `get_summoner_id_by_puuid()` then `get_raw_games()` (both with `blocking=False`, which uses `try_token()` instead of `wait_for_token()`), then scans the returned game list for a record whose integer `game["id"]` equals the numeric suffix of the Riot match ID (`int(match_id.split("_")[-1])`). If tokens are unavailable, `OpggRateLimitError` is raised immediately and the coordinator receives `THROTTLED` — no blocking, mirroring the RiotSource non-blocking contract. Non-match data types and requests missing a PUUID return `UNAVAILABLE` immediately without any network call.

`OpggMatchExtractor.extract()` overrides the `metadata.match_id` field set by `normalize_game()` in `_opgg_etl.py` with the Riot-format match ID passed by the coordinator, ensuring stored documents use the canonical `{PLATFORM}_{game_id}` key rather than the internal `OPGG_{platform}_{game_id}` key previously generated by the ETL layer.

`OpggClient.prefetch_player_games(puuid, opgg_region, blob_store, limit=20)` pre-populates BlobStore with a player's recent N games. It calls `get_summoner_id_by_puuid()` and `get_raw_games()` with `blocking=True` (acceptable for background paths), derives Riot-format match IDs (`f"{platform}_{game['id']}"`), and writes each blob via BlobStore. It returns the count of blobs written; individual errors are logged as warnings and return 0.

The primary delivery path for op.gg data is pre-fetch, not on-demand. When a player is searched in the UI (`/stats` route), if `opgg_enabled` and `BLOB_DATA_DIR` is set, a background `asyncio.create_task()` calls `prefetch_player_games(puuid, opgg_region, blob_store, limit=cfg.opgg_prefetch_limit)`. This pre-populates BlobStore at search time so the waterfall finds the player's recent games at step 2 (blob cache hit, zero op.gg HTTP calls) rather than triggering per-match on-demand fetches. On-demand fallback (`OPGG_FETCH_GAME_LIMIT`, default 5) covers matches queued after the pre-fetch window. A known amplification risk exists: N queued matches outside the pre-fetch window can each trigger a separate `get_raw_games()` call returning the same small payload; a per-player cache is a future optimization.

Game ID correlation: op.gg `game["id"]` is the Riot numeric game ID as an integer. `NA1_7234567890` corresponds to `game["id"] == 7234567890`. The module docstring previously claimed these could not be cross-referenced; that claim has been corrected.
...
| `OPGG_SUMMONER_CACHE_TTL_SECONDS` | `3600` | TTL (seconds) for Redis-cached PUUID-to-summoner-id lookups |
| `OPGG_FETCH_GAME_LIMIT` | `5` | Number of recent games fetched per on-demand waterfall request |
| `OPGG_PREFETCH_LIMIT` | `20` | Number of games pre-fetched to BlobStore on UI player search |
